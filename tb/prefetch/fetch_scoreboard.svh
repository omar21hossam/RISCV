...
task run_phase(uvm_phase phase ) ; 
  ...
  // // `uvm_error("FETCH_SB", $sformatf("FETCH done wrongly in normal op (no stall or flush),instr_id_o = %0d, instr_i = %0d", 
  // //                                 seq_item.instr_rdata_id_o, seq_item.instr_rdata_i))
  //  else 
...
`uvm_info("FETCH SB", $sformatf("BRANCH detected "),UVM_LOW)
 ...
 function instr_info_t classify_instruction(
    ...
    case (opcode)
      riscv_pkg::OPCODE_JAL: begin
        result.instr_type = INSTR_JUMP;
      end

      riscv_pkg::OPCODE_JALR: begin
          result.instr_type = INSTR_JUMP;
        
      end

      riscv_pkg::OPCODE_BRANCH: begin
...
//`uvm_info("FETCH SB", $sformatf("ADDRESS_CHECK_JB: inst add_f  %0d , inst add_d  %0d ",seq_item.pc_if_o,seq_item.pc_id_o),UVM_LOW)
...
// //`uvm_info("FETCH SB", $sformatf("ADDRESS_CHECK_NORMAL: Operation of inst fetch done correctly"),UVM_HIGH)
