...
//----------------------------------------------------------------------------------
      if (matched_op.instr_valid_id_o) 
              ...
              begin
                ...
                misaligned_inst[31:16] = matched_ip_2.instr_rdata_i[15:0];
                  if ((misaligned_inst == matched_op.instr_rdata_id_o)) 
                   ...
                   `uvm_info("FETCH SB", $sformatf("Misalignment done correctly "), UVM_HIGH)
                    misaligned ='b0;
                    num_passed++;
                    num_op=num_op+1;
                    num_op_total=num_op_total+1;
        ...
        case (info.instr_type)
          INSTR_JUMP: begin
                      ...
                      `uvm_info("FETCH SB", $sformatf("JUMP detected"), UVM_HIGH) 
                if (matched_op.instr_rdata_id_o[21] == 1'b1 )    begin

                      `uvm_info("FETCH SB", $sformatf("JUMP detected , misaligned add"), UVM_HIGH) 
          ...
          INSTR_BRANCH: begin
            ...
            `uvm_info("FETCH SB", $sformatf("BRANCH detected "), UVM_HIGH) 
          ...
          INSTR_JUMP_R: begin
            jump_expected = jump_expected + 1;
            `uvm_info("FETCH SB", $sformatf("JUMP_R detected"), UVM_HIGH)  
                        end
 ...
 `uvm_info("SCOREBOARD", $sformatf("\n\n====================== TEST SUMMARY ======================\n"), UVM_LOW)
    ...
    `uvm_info("SCOREBOARD", $sformatf("B-Type: %0d (%0.1f%%)", B_TYPE_cnt, (B_TYPE_cnt*100.0)/num_op_total), UVM_LOW)
    `uvm_info("SCOREBOARD", $sformatf("U-Type: %0d (%0.1f%%)", U_TYPE_cnt, (U_TYPE_cnt*100.0)/num_op_total), UVM_LOW)
    ...
    `uvm_info("SCOREBOARD", $sformatf("  XOR: %0d | OR: %0d | AND: %0d", XOR_cnt, OR_cnt, AND_cnt), UVM_LOW)
    `uvm_info("SCOREBOARD", $sformatf("  MUL: %0d | MULH: %0d | MULHU: %0d | MULHSU: %0d", 
              MUL_cnt, MULH_cnt, MULHU_cnt, MULHSU_cnt), UVM_LOW)
    `uvm_info("SCOREBOARD", $sformatf("  DIV: %0d | DIVU: %0d | REM: %0d | REMU: %0d", 
    ...
    `uvm_info("SCOREBOARD", $sformatf("  SRLI: %0d | SRAI: %0d", SRLI_cnt, SRAI_cnt), UVM_LOW)
    `uvm_info("SCOREBOARD", $sformatf("  JALR: %0d", JALR_cnt), UVM_LOW)
    `uvm_info("SCOREBOARD", $sformatf("  Loads: LB(%0d) LH(%0d) LW(%0d) LBU(%0d) LHU(%0d)", 
...
//***************************************************************************
  ...
  function check_add(input logic [31:0] add_f, input logic [31:0] add_d);
    ...
    begin
      ...
      else if ((matched_ip.pc_id_o != 0) && (matched_ip.pc_if_o != 0))
      begin
...
function instruction_types (input logic [31:0] instruction);
    ...
    unique case (opcode)
        riscv_pkg::OP_RTYPE: begin  
            ...
            else if (funct7 == riscv_pkg::SUB_SRA) begin
                unique case (funct3)
            ...
            else if (funct7 == riscv_pkg::R_OTHER) begin
                unique case (funct3)
