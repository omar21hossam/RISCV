class alu_coverage_collector extends uvm_subscriber #(alu_seq_item);
  ...
  covergroup alu_cov_inputs;
    ...
    vector_mode: coverpoint seq_item.vector_mode_i {
      bins vector_mode_0 = {0};
    }                 
...
endgroup:alu_cov_outputs
  ...
  function void report_phase(uvm_phase phase);
    ...
    `uvm_info(get_type_name(), $sformatf("Coverage Input: %0.2f%%", alu_cov_inputs.get_coverage()), UVM_MEDIUM)
    `uvm_info(get_type_name(), $sformatf("Coverage Output: %0.2f%%", alu_cov_outputs.get_coverage()), UVM_MEDIUM)
    endfunction
endclass
